...
export class PanelSummaryChartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //   this.panelSummaryChartService.updateData$.subscribe(d => {
    //     this.drawActive ? this.update(d) : this.draw(d)
    //   });
    //   this.panelSummaryChartService.drawData$.subscribe(d => {
    //     this.draw(d)
    //   });
  }
  ...
  activate(user, year) {
    this.data = null;
    this.budgetService.getUserSummary(name, year).subscribe(data => {
      this.data = data;
      if (this.drawActive) {
        this.update(data);
      }
